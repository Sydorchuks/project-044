...
export function EditUserPage({ userId }: EditUserPageProps) {
  ...
  return (
    <section className="bg-main-bg desktop:pr-10 ultra:pr-14 h-full min-h-0 overflow-auto px-5 pt-4 pb-8 xl:px-0 xl:pt-0 xl:pr-7">
      <div className="desktop:grid-cols-[390px_minmax(620px,640px)_1fr] desktop:gap-x-12 desktop:pt-14 ultra:grid-cols-[560px_minmax(700px,720px)_1fr] ultra:gap-x-16 ultra:pt-20 grid min-h-full w-full grid-cols-[250px_minmax(420px,460px)_1fr] grid-rows-[auto_1fr] gap-x-6 pt-12 xl:grid-cols-[300px_minmax(440px,500px)_1fr] xl:gap-x-8 xl:pt-10">
        ...
        <div
          className={cn("flex min-h-0 items-start", {
            "col-start-1 col-end-4 row-start-1 row-end-3 justify-center pt-22.5":
              isDeleted,
            "desktop:col-start-2 desktop:row-start-1 desktop:row-end-3 desktop:items-center desktop:pt-0 col-start-2 row-start-2 pt-10 xl:pt-8":
              !isDeleted,
          })}
        >
...
function EditUserFormFallback() {
  return (
    <div className="bg-background desktop:max-w-140 ultra:max-w-170 h-120 w-full max-w-115 animate-pulse rounded-3xl shadow-sm" />
  );
}
...
function EditUserError({
  ...
  return (
    <div className="bg-background desktop:max-w-140 ultra:max-w-170 w-full max-w-115 rounded-3xl p-8 text-center shadow-sm">
      <p className="text-text-error font-sans text-[14px] leading-5">{message}</p>
